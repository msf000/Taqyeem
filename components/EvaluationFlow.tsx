...
import { ChevronLeft, Save, Printer, ArrowRight, CheckCircle2, Loader2, AlertCircle, Calendar, ExternalLink, FileText, CheckSquare, TrendingUp, ThumbsUp, XCircle, LayoutList, MessageSquare, ChevronDown, ChevronUp, Target, Star, ArrowLeft, Maximize2 } from 'lucide-react';
...
export default function EvaluationFlow({ teacherId, evaluationId, onBack }: EvaluationFlowProps) {
  ...
  const handleSubCriteriaChange = (indicator: EvaluationIndicator, criteriaIdx: number, value: number) => {
      ...
      // Logic: Average of subScores (1-5) converted to weight
      const subScoreValues = Object.values(newSubScores);
      const sumSubScores = subScoreValues.reduce((a, b) => a + b, 0);
      
      // If indicator has sub-criteria, use them. Else (rare), use manual input logic
      let newMainScore = 0;
      let newLevel = 0;

      if (indicator.evaluationCriteria.length > 0) {
          // Average out of 5
          // We divide by number of criteria * 5 (max possible points) then multiply by weight
          ...
          // Determine level (1-5) based on average
          const avg = sumSubScores / indicator.evaluationCriteria.length;
          newLevel = Math.round(avg);
      } else {
          // Fallback if no sub-criteria defined (shouldn't happen in this view usually)
          newMainScore = value; 
      }

      // 3. Auto Text Generation
      let autoImprovement = currentScoreData.improvement || '';
      let autoStrengths = currentScoreData.strengths || '';
      
      if (newLevel === 5 && !autoStrengths) {
          autoStrengths = `أداء متميز في "${indicator.text}"، تطبيق احترافي للمعايير.`;
      } else if (newLevel <= 2 && !autoImprovement) {
          autoImprovement = `يحتاج إلى دعم وتوجيه في "${indicator.text}".`;
      }
      ...
      setScores(prev => ({
          ...
          [indicator.id]: {
              ...
              score: parseFloat(newMainScore.toFixed(2)), // Keep 2 decimals
              level: newLevel,
              isComplete: subScoreValues.length === indicator.evaluationCriteria.length, // Complete if all criteria rated
  ...
  const navigateIndicator = (direction: 'next' | 'prev') => {
      ...
      if (direction === 'next') {
          ...
          else setActiveIndicatorIndex(null); // Go back to list on finish
      } else {
                                ...
                                >
                                    ...
                                    <div className="flex justify-between items-end mt-4">
                                        <div className="text-sm">
                                            <span className="text-gray-500">الدرجة:</span> <span className={`font-bold ${isDone ? 'text-green-600' : 'text-gray-400'}`}>{scoreData.score.toFixed(1)}</span>
                                        </div>
                        ...
                        <div className="bg-white rounded-2xl shadow-xl border border-gray-200 overflow-hidden flex flex-col md:flex-row min-h-[600px]">
                            ...
                            <div className="w-full md:w-2/3 p-8 flex flex-col">
                                ...
                                <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-4 pt-6 border-t border-gray-100">
                                    <div>
                                        <label className="block text-xs font-bold text-green-700 mb-2">نقاط القوة</label>
                                        <textarea 
                                            ...
                                            placeholder="أبرز نقاط التميز..."
                                            value={scores[activeInd.id]?.strengths || ''}
                                            onChange={(e) => updateField(activeInd.id, 'strengths', e.target.value)}
                                        />
                                    </div>
                                    <div>
                                        <label className="block text-xs font-bold text-yellow-700 mb-2">فرص التحسين</label>
                                        <textarea 
                                            ...
                                            placeholder="المجالات التي تحتاج لتطوير..."
